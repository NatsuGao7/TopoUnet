# Leveraging Persistence Images to Enhance Robustness and Performance in Curvilinear Structure Segmentation (PI-Regressor & Topo SegNet)
...
##  📊 Originzing data
The persistence image (GT) generated in the previous step will be stored in the `PI` folder for later training.
The overall data organization is shown in the figure below.
```python
├── DRIVE
    ├──training
        ├──images
