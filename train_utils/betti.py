...
def calculate_betti(prediction,target,ignore_index):
    if ignore_index is not None:
        mask = target != ignore_index  # 仅保留非忽略区域
    ...
    betti_numbers, info_gudhi = get_topo_gudhi(target)
    target_betti_0 = betti_numbers[0]
    target_betti_1 = betti_numbers[1]

    prediction = torch.argmax(prediction, dim=1)  # Shape: (1, 584, 565)
    prediction = prediction.squeeze(0).cpu().numpy()
    prediction = prediction*mask
    betti_numbers, info_gudhi = get_topo_gudhi(prediction)
    prediction_betti_0 = betti_numbers[0]
    prediction_betti_1 = betti_numbers[1]

    betti_error_0= abs(target_betti_0 - prediction_betti_0)
    betti_error_1= abs(target_betti_1 - prediction_betti_1)
